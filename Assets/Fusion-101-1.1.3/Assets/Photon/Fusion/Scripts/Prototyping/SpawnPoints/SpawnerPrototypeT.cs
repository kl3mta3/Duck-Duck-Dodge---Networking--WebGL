...
public class SpawnerPrototype<T> : SimulationBehaviour, IPlayerJoined, IPlayerLeft, ISpawned, ISceneLoadDone where T : Component, ISpawnPointPrototype {
  ...
  public void TrySpawnAll() {
    ...
    public NetworkObject TrySpawnObject(NetworkRunner runner, NetworkObject _Object)
    {

        if (_Object == false/* || !_ScoreKeeper.IsValid*/)
        {
            return null;
        }
        else
        {
            ...
            return runner.Spawn(_Object, pos, Quaternion.identity, Object.InputAuthority);
        }
    }

    public void TrySpawnObjectForAll(NetworkObject _ScoreKeeper)
    {
        var runners = NetworkRunner.GetInstancesEnumerator();
        while (runners.MoveNext())
        {
            ...
            if (runner.IsRunning && runner.IsServer)
            {
                foreach (var p in runner.ActivePlayers)
                {
                    var ScoreKeeperObject = TrySpawnObject(runner, _ScoreKeeper);
                }
            }
        }
    }
