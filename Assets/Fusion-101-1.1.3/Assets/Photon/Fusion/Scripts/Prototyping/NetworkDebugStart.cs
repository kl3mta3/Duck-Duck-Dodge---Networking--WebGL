...
public class NetworkDebugStart : Fusion.Behaviour {
  ...
  public enum StartModes {
    ...
    //[InlineHelp]
   // [WarnIf(nameof(scoreKeeper), false, "No " + nameof(scoreKeeper) + " supplied. Will search for a " + nameof(ScoreKeeper) + " in the scene at startup.")]
  ...
  public void ShutdownAll() {
        Debug.Log("Shutting down all instances.");
    ...
    public void ToggleStartScreen()
    {
        ...
        {
            Debug.Log("Start Screen is Not active");
    ...
    public void ToggleScoreBars()
    {

        if (scoreBars.activeSelf)
        {
            scoreBars.SetActive(false);
        }
        else
        {
    ...
    RunnerPrefab = Instantiate(RunnerPrefab); 
        //scoreKeeper = Runner.Spawn(scoreKeeper);
